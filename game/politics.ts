...
function buildCrisis(): PendingCrisis {
  const def = randItem(CRISES);
...
export function advancePolitics(c: Character, log: LogEntry[]) {
  ...
  if (p.office) {
    ...
    // National crises land on executives' desks.
    if (p.office.executive && !p.pendingCrisis && Math.random() < 0.32) {
      p.pendingCrisis = buildCrisis();
      log.push({
        age: c.age,
